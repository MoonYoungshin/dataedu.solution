...
/** ◇◆ 토글. v0.8.11.0 | 20150520~ 20210908. 20220216. MoonYoungShin.
...
//(function doToggle(selector){
	$('.toggle').not('.donot').each(function(){ // 20150515. 토글 동작 안하기 추가
		...
		// 닫기 클릭. 20210826. 20210908. 20220216
...
(MJS.doSelectmenu1 = function(selector){
	...
	$('.selectmenu1').each(function(){
		...
		var $t1= $('.t1', $my); // 제목 텍스트
		var $that = $($my.attr('href')); // 옵션 래퍼 .selectmenu1option
		var $option = $('.option .a1', $that); // 옵션 요소
		var thatOn = 0; // 옵션 래퍼 focusout 고도화
		...
		$option.on('click', function(){
			...
			thatOn = 1; // 옵션 클릭으로 옵션 래퍼 focusout 되면서 0 으로 변한 값을 1 로 바꿔준다, $my.triggerHandler('click'); 가 재실행되는 것을 방지한다.
		...
		$that.on('focusout', function(){
			thatOn = 0;
			setTimeout(function(){
				if(!thatOn){ // 옵션 래퍼 focusin 되거나, 옵션 클릭으로 focusout 되는 경우는 실행안한다.
					$my.triggerHandler('click');
				}
			}, 50);
			return false;
