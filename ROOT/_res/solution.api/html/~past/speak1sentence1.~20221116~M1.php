...
<script>/*<![CDATA[*/
	$(function(){
		...
		/** ◇◆ 문장 순서 맞추기. 20210903. @m.
		 * 클릭한 순번 매김
		 * 다시 클릭하면 순번 제거
		 * 순번 모두 매기면 확인 버튼 생성
		 * 확인 버튼 누르면 순번대로 재배치하고 확인 버튼 사라짐
		 */
		(function(){
			var my = '.text1q1', // 래퍼
				ms = '.t1', // 문장
			...
			$(my).on('click', b1, function(e){
				// 설정한 순서로 재배치
				for(var i = 0; i <= mn; i++ ){
					$(my).find(m).filter('[data-sn="'+ i +'"]').appendTo( $(my).find(m).parent() );
				}
				// 초기화
				$(my).find(m)
					.removeClass('on')
			...
			function showHideBtn(){
				if( $(my).find(m).filter('.on').length >= mn ){ // 단어구 순번 모두 매겼으면
					$(my).find(b1).show();
				}else{
					$(my).find(b1).hide();
				}
