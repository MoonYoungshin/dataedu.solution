...
<script>/*<![CDATA[*/
	...
	/** ◇◆ 녹음시작종료. 20221111. 20221114. @m.
	 * 녹음중 애니메이션 CSS 대신 JS 로 수정
	 */
	(function(){

		//const $b1redord1 = document.querySelector('#btn-record-sw'); // 녹음시작/완료 버튼
		//const $audio1 = document.querySelector('#audio-e1');

		var $b1redord1 = $('#btn-record-sw'); // 녹음시작/완료 버튼
		...
		// 녹음시작/완료 버튼 클릭
		$b1redord1[0].onclick = async function (event) {
			if(!isRecording){
				...
				mediaRecorder.stop();
				isRecording = false;
				//$b1redord1.removeClass('recording');
				$('.bg1', $b1redord1).stop().animate({
						top: '-100%'
					}, 200, function(){
		...
		if(!isRecording){ // 녹음중 아니면
			$b1redord1.trigger('click'); // 녹음시작
		}
		// 완료 버튼을 눌리지 않을 경우 msec 초 후 종료
		msec += 900; // 녹음시간 상계
		clearTimeout(timer);
		timer = setTimeout(function(){
			if(!!isRecording){ // 녹음중
				$b1redord1.trigger('click'); // 녹음종료
			}
		}, msec); // 밀리초 지연
